...
final class Auth {
	...
	private static function authorization_header(): string {
		...
		// Compatibility: many hosts/proxies drop the Authorization header. Accept custom token headers too.
		if (!empty($_SERVER['HTTP_X_WPNEXUS_BRIDGE_TOKEN'])) {
			$h = 'Bearer ' . trim((string) $_SERVER['HTTP_X_WPNEXUS_BRIDGE_TOKEN']);
		} elseif (!empty($_SERVER['HTTP_X_WPNEXUS_TOKEN'])) {
			$h = 'Bearer ' . trim((string) $_SERVER['HTTP_X_WPNEXUS_TOKEN']);
		...
		if ($h === '' && function_exists('getallheaders')) {
			...
			if (is_array($headers)) {
				foreach ($headers as $k => $v) {
					...
					if ($h === '' && ($lk === 'x-wpnexus-bridge-token' || $lk === 'x-wpnexus-token')) {
						$h = 'Bearer ' . trim((string) $v);
						break;
					}
				}
			}
