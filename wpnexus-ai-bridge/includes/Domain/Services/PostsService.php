	...
	public function upsert(array $payload) {
		...
		if (!in_array($status, $allowed_status, true)) {
			$status = 'draft';
				...
				$title_raw = $this->decode_b64_field($payload, 'title');
		$content   = $this->decode_b64_field($payload, 'content');
		$excerpt   = $this->decode_b64_field($payload, 'excerpt');
...
$slug    = isset($payload['slug']) ? sanitize_title((string) $payload['slug']) : '';
	...
	private function decode_b64_field(array $payload, string $key): string {
		$bk = $key . '_b64';
		if (!empty($payload[$bk]) && is_string($payload[$bk])) {
