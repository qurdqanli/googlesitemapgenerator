...
final class MediaController {
	...
	public function upload(WP_REST_Request $request) {
		...
		if (empty($files['file'])) {
			$this->logger->warning('bridge.api.media.upload.missing_file', [
		...
		$this->logger->info('bridge.api.media.upload.file_meta', [
			'name'  => isset($files['file']['name']) ? (string) $files['file']['name'] : '',
			'type'  => isset($files['file']['type']) ? (string) $files['file']['type'] : '',
			'size'  => isset($files['file']['size']) ? (int) $files['file']['size'] : 0,
			'error' => isset($files['file']['error']) ? (int) $files['file']['error'] : -1,
		]);
