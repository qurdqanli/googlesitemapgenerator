...
final class UpsertTask implements TaskInterface {
	...
	public function handle(array $job_row): TaskResult {
		...
		// ---------------------------------------------------------
		// T14: Routing & Auto Resolve Logic
		// ---------------------------------------------------------
		// This block recalculates $lang and $send_original based on
		// post meta logic and target configuration.
		
		$router   = new \WPNexusAI\Engine\Routing\LanguageRouter();
		...
		$resolver = new \WPNexusAI\Engine\Routing\LanguageResolver();
		// Overwrite the initial $lang with the resolved one
		...
		if (!$send_original && !empty($payload['translation']['translations']) && is_array($payload['translation']['translations'])) {
		...
		// Term mapping (T13): map source terms to target term_ids via Bridge /terms/upsert
		$mapped_terms = null;
		...
		// If term mapping produced term_ids, use them (Bridge accepts term_id items).
		...
		// Idempotent skip: if linked and same hash -> no remote call (unless forced by reconcile)
		if (!$force && $prev_state === 'linked' && $prev_hash !== '' && hash_equals($prev_hash, $hash)) {
			...
			// Optional: store a note for UI debugging
			$payload['upsert'] = [
				'skipped' => true,
		...
		if (!$res->ok) {
			...
			// Bridge missing -> needs_input (admin action)
			if ($status === 404) {
