	...
	public function run(int $job_id, array $job_row): TaskResult {
		...
		if ($target_id > 0) {
			...
			if ($target_row) {
				...
				// If send_original is enabled, skip translation
				if ($send_original) {
					// Update payload with resolved languages for chaining
					$payload['send_original'] = 1;
				...
				// Update payload with resolved languages for use below
				$payload['source_lang'] = $source_lang;
				$payload['target_lang'] = $target_lang;
		...
		// Skip translation if source/target are the same (explicitly)
		$src_norm = Lang::sanitize_code($source_lang);
		$tgt_norm = Lang::sanitize_code($target_lang);
		if ($src_norm !== '' && $tgt_norm !== '' && $src_norm !== 'auto' && $tgt_norm !== 'auto' && $src_norm === $tgt_norm) {
			$payload['translation'] = [
				'provider'     => '',
				'key_id'       => 0,
				'translations' => [],
				'usage'        => [],
				'note'         => 'Source and target language are the same; translation skipped.',
			];
			$this->jobs->update_payload($job_id, $payload);
			...
			if ($chain_upsert) {
				...
				$this->logger->info('task.translate.chain.upsert', [
					'job_id'      => $job_id,
					'next_job_id' => $next_job_id,
					'note'        => 'Same language; translation skipped.',
				]);
			}
