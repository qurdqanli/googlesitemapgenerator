...
final class MappingTermsRepo extends Repo {
	...
	public function get(int $target_id, string $taxonomy, int $source_term_id, string $language_code): ?array {
		...
		$row = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$table} WHERE target_id = %d AND taxonomy = %s AND source_term_id = %d AND language_code = %s",
				$target_id,
				$taxonomy,
				$source_term_id,
				$language_code
	...
	public function upsert(
		...
		$sql = "
			INSERT INTO {$table}
				(target_id, taxonomy, source_term_id, language_code, target_term_id, slug)
			VALUES
				(%d, %s, %d, %s, %d, %s)
